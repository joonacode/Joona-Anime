...
class GenreResult extends HTMLElement {

    set genreres(genreres) {
        this._genreres = genreres;
        this.render();
    }
    ...
    render() {
        ...
        row.classList.add('owl-carousel', 'owl-theme', 'genreres');
        this.appendChild(row);

        this._genreres.forEach(e => {
            card += this.renderCard(e);
            row.innerHTML = card;
        });
        $('.genreres').owlCarousel({
            nav: true,
            stagePadding: 50,
            lazyLoad: true,
            dots: false,
...
customElements.define('genre-result', GenreResult);